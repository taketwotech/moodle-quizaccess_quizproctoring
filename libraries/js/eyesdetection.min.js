(function () {
    ...
    async function setupFaceMesh(videoElement, canvasElement, faceMesh, detectionval, callback, enableobjectdetect, enableeyecheckreal) {
        ...
        if (enableobjectdetect === 1 && objectDetector) {
            // Wait for video to be ready
            await waitForVideoReady(videoElement);
            
            // Start object detection (detectObjects handles its own loop internally)
            runObjectDetection(videoElement, canvasElement);
        }
        ...
        // Only run eye-tracking logic when eye tracking is enabled; otherwise only object detection can trigger warnings
        const eyeTrackingEnabled = enableeyecheckreal === 1 || enableeyecheckreal === true;
        ...
        faceMesh.onResults((results) => {
            ...
            if (eyeTrackingEnabled && results.multiFaceLandmarks?.length === 1) {
    ...
    const OBJECT_DETECTION_COOLDOWN = 3000;
    const lastDetected = {
        'cell phone': 0,
        'mobile phone': 0,
        'book': 0
    ...
    async function initializeObjectDetection() {
        if (objectDetectionInitialized || objectDetector) {
            return objectDetector;
        }

        try {
            // Import object detector module
            const { initializeObjectDetector, detectObjects, captureImage } = await import(
                M.cfg.wwwroot + '/mod/quiz/accessrule/quizproctoring/libraries/js/objectdetector.js'
            );

            // Wait a bit to ensure face mesh WASM is stable
            await new Promise(resolve => setTimeout(resolve, 2000));

            await initializeObjectDetector();
            objectDetector = { detectObjects, captureImage };
    ...
    function runObjectDetection(video, canvas) {
        if (!objectDetector || !video || video.readyState < 2) {
            return;
        }

        objectDetector.detectObjects(video, function (result) {
            const now = Date.now();
            const TARGET_OBJECTS = ['book', 'cell phone', 'mobile phone'];

            const labels = result.detections
                .filter(d => {
                    const label = d.categories[0].categoryName.toLowerCase();
                    const score = d.categories[0].score;
                    if (label === 'book' && score >= 0.2) return true;
                    if ((label === 'cell phone' || label === 'mobile phone') && score >= 0.5) return true;
                    return false;
                })
                .map(d => d.categories[0].categoryName.toLowerCase())
                .filter(label => TARGET_OBJECTS.includes(label));

            console.log('Detected objects:', labels);

            const triggered = labels.some(label => {
                if (lastDetected.hasOwnProperty(label)) {
                    if (now - lastDetected[label] > OBJECT_DETECTION_COOLDOWN) {
            ...
            if (triggered && window.objectDetectionCallback) {
                const cleanImage = objectDetector.captureImage(video);
                window.objectDetectionCallback({
                    status: 'objectsdetected',
                    data: cleanImage
                });
            }
        }, canvas);
    ...
    window.initializeObjectDetection = initializeObjectDetection;
})();