...
/**
 * Sync ProctorLink plan and user info when cached data is stale.
 *
 ...
 */
function quizaccess_quizproctoring_sync_plan_if_stale(bool $force = false, ?callable $logger = null): array {
    return \quizaccess_quizproctoring\plan_sync::sync_if_stale($force, $logger);
}

/**
 * Build the URL used to force-refresh plan details.
 *
 * @param int $cmid Course-module id when refreshing from quiz settings; 0 for admin settings.
 * @return moodle_url
 */
function quizaccess_quizproctoring_plan_refresh_url(int $cmid = 0): moodle_url {
    $params = [
        'sesskey' => sesskey(),
        'returnto' => $cmid > 0 ? 'quiz' : 'admin',
    ];
    if ($cmid > 0) {
        $params['cmid'] = $cmid;
    }
    return new moodle_url('/mod/quiz/accessrule/quizproctoring/refreshplan.php', $params);
}

/**
 * Render plan status HTML for admin and quiz settings pages.
 *
 * @param moodle_url|null $refreshurl When set, a refresh link is shown.
 * @return string
 */
function quizaccess_quizproctoring_render_plan_status(?moodle_url $refreshurl = null): string {
    ...
    $note = ($planinfo == 1 && $planname === 'Credit Base Plan')
        ? get_string('creditbalanceupdatenote', 'quizaccess_quizproctoring')
        : get_string('updatenote', 'quizaccess_quizproctoring');

    $planstatus .= '<div class="plan-info-note">' . $note;
    if ($refreshurl !== null) {
        $planstatus .= ' <span class="plan-separator">|</span> '
            . html_writer::link(
                $refreshurl,
                get_string('refreshplan', 'quizaccess_quizproctoring'),
                ['class' => 'plan-refresh-link']
            );
    }
    $lastsync = \quizaccess_quizproctoring\plan_sync::get_last_sync_time();
    if ($lastsync > 0) {
        $planstatus .= ' <span class="plan-separator">|</span> '
            . get_string('plansynclastsynced', 'quizaccess_quizproctoring', userdate($lastsync));
    }
    $planstatus .= '</div>';
